  ...
  function exportCsv() {
    ...
    for (const a of plan.actions) {
      rows.push([
        a.ticker,
        a.side,
        String(a.delta_quantity),
        String(a.target_quantity),
        String(a.target_weight),
        String(a.current_weight),
        a.est_cost,
        a.rationale,
      ]);
    }
    ...
    a.href = url;
    a.download = `rebalance_${pid}_${plan.strategy_id}.csv`;
    a.click();
    ...
    <div className="w-full space-y-4">
      <Card className={cn("flex items-end gap-2 p-4")}>
        <div className="flex-1">
          ...
          <select
            id="strategy-select"
            className={cn("w-full rounded border px-2 py-1")}
        ...
        <Button onClick={run} disabled={running || !selectedStrategy}>
          {running ? "生成中..." : "生成调仓建议"}
        </Button>
      </Card>
      {plan && (
        <>
          {plan.warnings.length > 0 && (
            <Card className={cn("border-yellow-200 bg-yellow-50 p-3")}>
              {plan.warnings.map((w) => (
                <div key={w} className="text-sm text-yellow-800">
                  ⚠ {w}
                </div>
              ))}
            </Card>
          ...
          {plan.fund_transfers.length > 0 && (
            <Card className={cn("border-blue-200 bg-blue-50 p-3")}>
              ...
              {plan.fund_transfers.map((ft) => (
                <div
                  key={`${ft.from_portfolio_id}-${ft.to_portfolio_id}-${ft.amount}`}
                  className="text-blue-800 text-sm"
                >
                  ¥{Number(ft.amount).toLocaleString()}：{ft.rationale}
                </div>
              ))}
            </Card>
          )}
          <Card className="p-4">
            <div className={cn("mb-2 flex items-center justify-between")}>
              ...
              <Button variant="outline" size="sm" onClick={exportCsv}>
                <Download className={cn("mr-1 h-4 w-4")} /> 导出 CSV
              </Button>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500">
                  <th>代码</th>
                  <th>方向</th>
                  <th className="text-right">调整量</th>
                  <th className="text-right">目标量</th>
              ...
              <tbody>
                {plan.actions.map((a) => (
                  <tr key={a.ticker} className="border-t">
                    ...
                    <td
                      className={cn(
                        a.side === "buy" ? "text-red-500" : "text-green-500",
                      )}
                    >
                      {a.side === "buy" ? "买入" : "卖出"}
                    </td>
                    ...
                    <td className="text-right">
                      {(a.target_weight * 100).toFixed(2)}%
                    </td>
                    <td className="text-right">
                      {(a.current_weight * 100).toFixed(2)}%
                    </td>
                    <td className="text-right">
                      ¥{Number(a.est_cost).toFixed(2)}
                    </td>
            ...
            <div className={cn("mt-3 space-y-1 border-t pt-3 text-sm")}>
              <div className="flex justify-between">
                <span>总交易成本</span>
                <span>¥{Number(plan.total_est_cost).toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span>现金变化</span>
                <span>
                  ¥{Number(plan.cash_before).toFixed(2)} → ¥
                  {Number(plan.cash_after).toFixed(2)}
                </span>
              </div>
            </div>
          </Card>
