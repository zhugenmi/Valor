          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-gray-500">
                ...
                <th>名称</th>
                <th>方法</th>
                <th className="text-right">期望收益</th>
              ...
              {current.strategies.map((s) => (
                <tr key={s.strategy_id} className="border-t">
                  <td>
                    <input
                      type="checkbox"
                      checked={selected.has(s.strategy_id)}
                  ...
                  <td className="text-right">
                    {s.expected_volatility != null
                      ? `${(s.expected_volatility * 100).toFixed(2)}%`
                      : "-"}
                  </td>
            ...
            <thead>
              <tr className="text-left text-gray-500">
                ...
                {current.strategies
                  ...
                  .map((s) => (
                    <th key={s.strategy_id}>{s.name}</th>
                  ))}
              </tr>
            </thead>
            <tbody>
              {Array.from(
                new Set(
                  current.strategies
                    .filter((s) => selected.has(s.strategy_id))
                    .flatMap((s) => Object.keys(s.target_weights)),
                ),
              ).map((t) => (
                <tr key={t} className="border-t">
                  ...
                  {current.strategies
                    .filter((s) => selected.has(s.strategy_id))
                    .map((s) => (
                      <td key={s.strategy_id} className="text-right">
                        {((s.target_weights[t] || 0) * 100).toFixed(2)}%
                      </td>
                    ))}
                </tr>
              ))}
